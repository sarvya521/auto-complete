...
public final class LoggerUtilities {
     ...
     * format and the parameters will be cleansed by replacing the CR and LF characters with
     * the Java strings "\\r" and "\\n".
     * The parameters are merged within the messages format through the MessageFormat class.
     * @see MessageFormat
     * @param messageFormat the message pattern with place holders; both indexed and non-indexed
     * place holders are accepted.
     * Examples:
     *     "Message first text. Message second text  {0} {1}"
     *     "Message first text. Message second text  {} {}"
     * @param formatParams the formatting arguments. If the message is fixed text and doesn't
     * have place holders, formatParams shall be null or empty array.
     * @return formatted message string
     */
    public static String getMessage(final String messageFormat,
    ...
    /**
     * Prepares the parameters for inserting in the message template.
     * Optionally could apply OWASP Encoder for Java to replace the
     * CR and the LF characters by the Java strings "\\r" and "\\n".
     * @param cleanCRLF - if true will call Encoder. Encoder shall not be called if the
     * entire message is being cleansed.
     * @param formatParams params to sanitise
     * @return cleansed versions of params.
     */
    private static Object[] sanitiseParams(boolean cleanCRLF,
                                           final Object... formatParams) {
    ...
    /**
     * Replaces all occurrences of "{}" with "{0}", "{1}", etc. so that the text can be
     * message pattern compliant to MessageFormat.format. The method doesn't apply CRLF cleaning.
     * DO NOT use the method outside of the class, its package-visible
     * only for the sake of the unit testing.
     *
     * @see MessageFormat
     ...
     */
    private static String getFormat(boolean addIndexes,
                            final String format) {
